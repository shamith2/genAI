# Llama 2 for ONNX
# Adapted to work with ONNX and ONNXRuntime
...
class ModelArgs:
    ...
    norm_eps: float = 1e-5 # to avoid division-by-zero in RMSNorm
    
    hidden_dim: int = 11008 # 14336 for mistral 7b
    # window_size: int = 4096

    max_batch_size: int = 1 # batch size = 1 for inference
...
class MultiHeadAttention(torch.nn.Module):
    ...
    def __init__(self, args: ModelArgs):
        ...
        # self.register_buffer('cache_k', torch.zeros((args.max_batch_size, self.n_kv_heads, 0, self.head_dim)))        
        # self.register_buffer('cache_v', torch.zeros((args.max_batch_size, self.n_kv_heads, 0, self.head_dim)))
    ...
    def forward(
        ...
        # KV cache
        keys = torch.cat((cache_k, xk), dim=2) # (bs, n_kv_heads, cache_len + seqlen, head_dim)
        values = torch.cat((cache_v, xv), dim=2) # (bs, n_kv_heads, cache_len + seqlen, head_dim)
        ...
        scores = torch.matmul(xq, keys.transpose(2, 3)) / math.sqrt(self.head_dim) # torch.sqrt(torch.tensor([self.head_dim]))
...
class DecoderBlock(torch.nn.Module):
    ...
    def forward(
        ...
        _out, cache_k, cache_v = self.attention.forward(
...
class Transformer(torch.nn.Module):
    def __init__(self, args: ModelArgs):
        ...
        # self.register_buffer('k', torch.zeros((self.params.max_batch_size, self.params.n_kv_heads, 0, self.params.dim // self.params.n_heads)))
        # self.register_buffer('v', torch.zeros((self.params.max_batch_size, self.params.n_kv_heads, 0, self.params.dim // self.params.n_heads)))
    ...
    def forward(
        ...
        # self.token_pos.add_(seqlen) => might work with aten operators

        for i, layer in enumerate(self.layers):
            # torch.save(h, os.path.join(save_dir, 'h_' + str(i) + '.pt'))
            
            h, k, v = layer(h, freqs_cos, freqs_sin, keys[i], values[i])
...
if __name__ == '__main__':
    ...
    cache_k = [torch.zeros((args.max_batch_size, args.n_kv_heads, 0, args.dim // args.n_heads))] * args.n_layers
    cache_v = [torch.zeros((args.max_batch_size, args.n_kv_heads, 0, args.dim // args.n_heads))] * args.n_layers
